...
function calculateDistance($cities, $path) {
    $distance = 0;
    ...
    for ($i = 0; $i < $numCities - 1; $i++) {
        ...
        // 计算当前城市和下一个城市之间的距离
        $distance += calculateDistanceBetweenCities($currentCity, $nextCity);
    }

    // 返回总距离以及回到起点的距离
    $distance += calculateDistanceBetweenCities($cities[$path[$numCities - 1]], $cities[$path[0]]);
...
function findShortestPath($cities) {
    ...
    $shortestDistance = calculateDistance($cities, $shortestPath);

    // 生成所有可能的路径
    $permutations = permute(range(0, $numCities - 1));

    // 遍历所有路径，计算每个路径的总距离并更新最短路径和距离
    foreach ($permutations as $path) {
        $distance = calculateDistance($cities, $path);
...
function permute($elements) {
    ...
    foreach ($elements as $index => $element) {
        ...
        foreach (permute($rest) as $permutation) {
...
// 示例使用
$cities = ['A', 'B', 'C', 'D'];
$shortestPath = findShortestPath($cities);
$shortestDistance = calculateDistance($cities, $shortestPath);
